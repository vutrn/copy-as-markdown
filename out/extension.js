...
class Provider {
    ...
    async load() {
        ...
        const exclude = vscode.workspace.getConfiguration('copyAsMarkdown').get('exclude', '**/{node_modules,.git,.svn,.hg,dist,build,out}/**');
        const uris = await vscode.workspace.findFiles('**/*', exclude);
        const result = [];
        for (const uri of uris) {
            if (!this.isCodeFile(uri))
                continue;
            result.push({
                kind: 'file',
                uri,
        ...
        if (item instanceof ControlNode) {
            if (state === vscode.TreeItemCheckboxState.Checked) {
                for (const file of this.files)
                    this.selected.add(file.uri.toString());
            }
            else {
                this.selected.clear();
            }
    ...
    isCodeFile(uri) {
        ...
        if (SPECIAL_CODE_NAMES.has(name))
            return true;
    ...
    getFolders(parent) {
        ...
        for (const file of this.files) {
            ...
            if (slash >= 0)
                names.add(prefix + rest.slice(0, slash));
        }
...
function activate(context) {
    ...
    context.subscriptions.push(tree.onDidChangeCheckboxState((event) => {
        for (const entry of event.items) {
            const item = (Array.isArray(entry) ? entry[0] : entry);
            provider.toggleChecked(item);
        }
    }), vscode.commands.registerCommand('copyAsMarkdown.copySelected', () => copySelected(provider)), vscode.commands.registerCommand('copyAsMarkdown.refresh', () => provider.refresh()), vscode.commands.registerCommand('copyAsMarkdown.clearSelection', () => provider.clearSelection()), vscode.commands.registerCommand('copyAsMarkdown.selectAll', () => provider.selectAll()), vscode.workspace.onDidChangeWorkspaceFolders(() => provider.refresh()));
